...
foreach ($results as $row){
	echo('<tr><td>'.$row['nome'].'</td><td>'.$row['sconto'].'</td><td>'.$row['prezzo'].'</td><td>'.$row['provvigionedefault'].'</td><td>'.$row['percentualecapo'].'</td><td><a href="index.php?section=insertproduct&action=mod&id='.$row['id'].'">modifica</a></td></tr>');
}
echo('</table> </div>');	
?>
